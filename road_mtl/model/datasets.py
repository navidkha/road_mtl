...
#         for frame_index, frame_num in enumerate(
#                 np.arange(tube['sf'], tube['ef'], 1)):  # start of the tube to end frame of the tube
#             label = tube['label']
...
#
#         ratios = [1.0, 1.1, 1.1, 0.9, 1.1, 0.8, 0.7, 0.8, 1.1, 1.4, 1.0, 0.8, 0.7, 1.2, 1.0, 0.8, 0.7, 1.2, 1.2, 1.0,
#                   0.9]
#
#         self.video_list = []
#         self.numf_list = []
...
#                     frames_with_boxes += 1
#                 frame_level_annos[frame_index]['labels'] = np.asarray(frame_level_annos[frame_index]['labels'],
#                                                                       dtype=np.float32)
#                 frame_level_annos[frame_index]['boxes'] = np.asarray(frame_level_annos[frame_index]['boxes'],
#                                                                      dtype=np.float32)
#
...
#                 step_list = [s for s in range(self.MIN_SEQ_STEP, self.MAX_SEQ_STEP + 1) if
#                              numf - s * self.SEQ_LEN >= frame_num]
#                 shuffle(step_list)
...
#
#             frames = database[videoname]['frames']
#             frame_level_annos = [{'labeled': False, 'ego_label': -1, 'boxes': np.asarray([]), 'labels': np.asarray([])}
#                                  for _ in range(numf)]
#
#             frame_nums = [int(f) for f in frames.keys()]
#             frames_with_boxes = 0
#             for frame_num in sorted(
#                     frame_nums):  # loop from start to last possible frame which can make a legit sequence
...
#                             filtered_ids = filter_labels(anno[name + '_ids'], final_annots['all_' + name + '_labels'],
